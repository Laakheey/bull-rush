...
import { useUserFinanceData } from '../../hooks/useROILogs'; // updated hook name
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 mb-12">
          <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center">
            ...
            <span className="text-sm text-gray-400">{rewards.length} entries</span>
          </div>
                ...
                {rewards.map((reward) => (
                  <tr key={reward.id} className="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td className="px-8 py-5">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${getPlanBadge(
                          reward.plan_type
                        )}`}
                      >
                        {reward.plan_type || 'N/A'}
                      </span>
                    </td>
                    ...
                    <td className="px-8 py-5 text-right font-black text-green-600">
                      +{Number(reward.amount).toLocaleString()} USDT
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
          <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center">
            ...
            <span className="text-sm text-gray-400">{withdrawals.length} entries</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-50">
                {withdrawals.map((wd) => (
                  <tr key={wd.id} className="hover:bg-indigo-50/30 transition-colors">
                    <td className="px-8 py-5 text-sm text-gray-600">
                      {new Date(wd.created_at).toLocaleDateString()}
                    </td>
                    <td className="px-8 py-5">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${getStatusBadge(
                          wd.status
                        )}`}
                      >
                        {wd.status || 'unknown'}
...
const StatCard = ({ title, value, color }: any) => (
  <div className="bg-white p-6 rounded-3xl shadow-md border border-gray-100">
    ...
    <p className={`text-2xl font-black ${color}`}>
      {value.toLocaleString()} <span className="text-xs">USDT</span>
    </p>
  </div>
);

// Existing getPlanBadge
